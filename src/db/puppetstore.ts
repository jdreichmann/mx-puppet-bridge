...
export class DbPuppetStore {
	...
	public async deleteStatusRoom(mxid: string) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "update_status", table: "puppet_store",
		});
	...
	public async getMxidInfo(puppetMxid: string): Promise<IMxidInfo | null> {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "get_mx_info", table: "puppet_store",
		});
	...
	public async setMxidInfo(puppet: IMxidInfo) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "set_mxid_info", table: "puppet_store",
		});
	...
	public async getAll(): Promise<IPuppet[]> {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "select_all", table: "puppet_store",
		});
		let result: IPuppet[] = [];
	...
	public async getForMxid(puppetMxid: string): Promise<IPuppet[]> {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "select_for_mx", table: "puppet_store",
		});
	...
	public async get(puppetId: number): Promise<IPuppet | null> {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "select", table: "puppet_store",
		});
		const cached = this.puppetCache.get(puppetId);
		if (cached) {
			return cached;
	...
	public async getMxid(puppetId: number): Promise<string> {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "select_mxid", table: "puppet_store",
		});
	...
	public async setUserId(puppetId: number, userId: string) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "update_uid", table: "puppet_store",
		});
	...
	public async setData(puppetId: number, data: IPuppetData) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "update_data", table: "puppet_store",
		});
		let dataStr = "";
		try {
	...
	public async setType(puppetId: number, type: PuppetType) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "update_type", table: "puppet_store",
		});
	...
	public async setIsPublic(puppetId: number, isPublic: boolean) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "update_visibility", table: "puppet_store",
		});
	...
	public async setAutoinvite(puppetId: number, autoinvite: boolean) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "update_autoinvite", table: "puppet_store",
		});
	...
	public async setIsGlobalNamespace(puppetId: number, isGlobalNamespace: boolean) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "update_namespace", table: "puppet_store",
		});
	...
	public async new(
		...
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "insert", table: "puppet_store",
		});
		let dataStr = "";
		try {
			dataStr = JSON.stringify(data);
	...
	public async delete(puppetId: number) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "delete", table: "puppet_store",
		});
	...
	public async isGhostInRoom(ghostMxid: string, roomMxid: string): Promise<boolean> {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "select_ghost_in_room", table: "puppet_store",
		});
	...
	public async joinGhostToRoom(ghostMxid: string, roomMxid: string) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "insert_ghost_to_room", table: "puppet_store",
		});
	...
	public async getGhostsInRoom(room: string): Promise<string[]> {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "select_all_ghost_in_room", table: "puppet_store",
		});
		const result: string[] = [];
	...
	public async getRoomsOfGhost(ghost: string): Promise<string[]> {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "select_all_rooms_of_ghost", table: "puppet_store",
		});
		const result: string[] = [];
	...
	public async emptyGhostsInRoom(room: string) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "delete_ghosts_in_room", table: "puppet_store",
		});
	...
	public async leaveGhostFromRoom(ghostMxid: string, roomMxid: string) {
		const stopTimer = this.db.latency.startTimer({
			engine: this.db.type, type: "delete_ghost_in_room", table: "puppet_store",
		});
